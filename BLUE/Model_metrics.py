...
class ModelEvaluator(object):
    ...
    def one_step_eval_num(self, y_pred_step: np.array, y_true_step: np.array, current_step_idx: int, run_plots_dir: str, mode_name: str):
        ...
        MSEs, RMSEs, MAEs, MAPEs, F1s, Precisions, Recalls, Pearsons, Spearmans = [], [], [], [], [], [], [], [], []
        ...
        for c in range(y_pred_step.shape[-1]):
            if c == 0:
                ...
                print(f'MSE: {MSE_c:11.4f}, RMSE: {RMSE_c:9.4f}, MAE: {MAE_c:9.4f}, MAPE: {MAPE_c:9.4%}, F1: {F1_c:9.4f}, Precision: {Precision_c:9.4f}, Recall: {Recall_c:9.4f}, Pearson: {Pearson_c:9.4f}, Spearman: {Spearman_c:9.4f}')
                MSEs.append(MSE_c)
                RMSEs.append(RMSE_c)
                MAEs.append(MAE_c)
                MAPEs.append(MAPE_c)
        ...
        print('Overall: \n'
              ...
              f'   Recall: {step_metrics["recall"]:9.4f}, Pearson: {step_metrics["pearson"]:9.4f} \n'
              f'   Spearman: {step_metrics["spearman"]:9.4f} \n')
    ...
    def precision_score(y_pred: np.array, y_true: np.array):
        """
        Calculate precision score based on the presence of cases.
        Only considers whether a location has cases or not (binary), not the actual count.
        """
        if hasattr(y_pred, 'detach') and hasattr(y_pred, 'cpu') and hasattr(y_pred, 'numpy'):
            y_pred = y_pred.detach().cpu().numpy()
    ...
    def recall_score(y_pred: np.array, y_true: np.array):
        """
        Calculate recall score based on the presence of cases.
        Only considers whether a location has cases or not (binary), not the actual count.
        """
        if hasattr(y_pred, 'detach') and hasattr(y_pred, 'cpu') and hasattr(y_pred, 'numpy'):
            y_pred = y_pred.detach().cpu().numpy()
    ...
    def spearman_corr(y_pred: np.array, y_true: np.array):
        """
        Calculate Spearman rank correlation coefficient between predictions and ground truth.
        Spearman correlation is better at capturing non-linear patterns and monotonic relationships.
        """
        if hasattr(y_pred, 'detach') and hasattr(y_pred, 'cpu') and hasattr(y_pred, 'numpy'):
            y_pred = y_pred.detach().cpu().numpy()
        if hasattr(y_true, 'detach') and hasattr(y_true, 'cpu') and hasattr(y_true, 'numpy'):
