...
class FusionGraphBuilder(nn.Module):
    ...
    def generate_fusion_nodes(self, x_dict, edge_index_dict):
        ...
        if 'case' in x_dict and ('case', 'belongs_to', 'county') in edge_index_dict:
            ...
            # 只选择有belongs_to连接的case
            selected_case_features = case_features[case_indices]
            selected_genetic_features = case_genetic_features[case_indices]
            
            enhanced_case_features = torch.cat([selected_case_features, selected_genetic_features], dim=1)
    ...
    def transform_hetero_edges_to_fusion(self, edge_index_dict, fusion_mapping, case_indices, county_indices, fusion_features=None):
        ...
        if ('case', 'genetic', 'case') in edge_index_dict and ('case', 'belongs_to', 'county') in edge_index_dict:
            ...
            if belongs_to_edges.size(1) > 0:
                num_cases = torch.max(belongs_to_edges[0]) + 1
            else:
                num_cases = 0
            ...
            # 添加边界检查
            if genetic_edges.size(1) > 0 and num_cases > 0:

                
                # 确保索引在有效范围内 - 使用try-catch来安全处理
                try:
                    # 首先检查索引范围
                    valid_src_indices = (genetic_edges[0] >= 0) & (genetic_edges[0] < num_cases)
                    valid_dst_indices = (genetic_edges[1] >= 0) & (genetic_edges[1] < num_cases)
                    
                    # 创建临时掩码
                    temp_mask = valid_src_indices & valid_dst_indices
                    
                    if temp_mask.any():
                        # 安全地获取有效的case索引
                        valid_src_cases = genetic_edges[0, temp_mask]
                        valid_dst_cases = genetic_edges[1, temp_mask]
                        ...
                        # 检查county映射是否有效
                        valid_county_mask = (src_counties >= 0) & (dst_counties >= 0)
                        
                        # 创建最终的掩码
                        valid_genetic_mask = torch.zeros(genetic_edges.size(1), dtype=torch.bool, device=device)
                        valid_genetic_mask[temp_mask] = valid_county_mask
                    else:
                        valid_genetic_mask = torch.zeros(genetic_edges.size(1), dtype=torch.bool, device=device)
                        
                except Exception as e:
                    print(f"Error in genetic edge processing: {e}")
                    valid_genetic_mask = torch.zeros(genetic_edges.size(1), dtype=torch.bool, device=device)
...
class FusionGNN(nn.Module):
    ...
    def __init__(self, channel=2, hidden_dim=64, num_layers=2, dropout=0.3, pred_horizon=4,
        ...
        for _ in range(num_layers):
            conv = SAGEConv((-1, -1), hidden_dim).to(self.device)
