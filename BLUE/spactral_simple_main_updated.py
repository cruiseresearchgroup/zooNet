...
def compute_full_heterogeneous_laplacian(x_dict, edge_index_dict, device):
    """
    Compute the full heterogeneous Laplacian including both county and case nodes.
    This aligns with the theoretical framework where L_hetero is of size (N + M_max) x (N + M_max).
    """
    if 'county' not in x_dict:
        logger.warning("County nodes not found for Hetero Laplacian.")
        return None

    county_features = x_dict['county']
    num_counties = county_features.size(0)
    
    # Handle case nodes if they exist
    num_cases = 0
    if 'case' in x_dict:
        case_features = x_dict['case']
        num_cases = case_features.size(0)
    
    total_nodes = num_counties + num_cases
    
    if total_nodes == 0:
        logger.warning("No nodes found for Hetero Laplacian computation.")
        return None

    # Initialize full adjacency matrix
    adj = torch.zeros((total_nodes, total_nodes), device=device)
    
    # Add county-county spatial connections
    ...
    # Add case-case genetic connections if they exist
    if num_cases > 0 and ('case', 'genetic', 'case') in edge_index_dict:
        genetic_edges = edge_index_dict[('case', 'genetic', 'case')]
        if genetic_edges.size(1) > 0:
            src, dst = genetic_edges
            # Offset case indices by num_counties
            src_offset = src + num_counties
            dst_offset = dst + num_counties
            valid_mask = (src_offset < total_nodes) & (dst_offset < total_nodes)
            src_offset, dst_offset = src_offset[valid_mask], dst_offset[valid_mask]
            adj[src_offset, dst_offset] = 1
            adj[dst_offset, src_offset] = 1
    
    # Add case-county belongs_to connections if they exist
    if num_cases > 0 and ('case', 'belongs_to', 'county') in edge_index_dict:
        belongs_to_edges = edge_index_dict[('case', 'belongs_to', 'county')]
        if belongs_to_edges.size(1) > 0:
            case_src, county_dst = belongs_to_edges
            # Offset case indices by num_counties
            case_src_offset = case_src + num_counties
            valid_mask = (case_src_offset < total_nodes) & (county_dst < num_counties)
            case_src_offset, county_dst = case_src_offset[valid_mask], county_dst[valid_mask]
            adj[case_src_offset, county_dst] = 1
            adj[county_dst, case_src_offset] = 1
...
def compute_county_spatial_laplacian(x_dict, edge_index_dict, device):
    """
    Legacy function - compute only county spatial Laplacian.
    Use compute_full_heterogeneous_laplacian for theoretical alignment.
    """
...
def compute_projected_fusion_laplacian(fusion_features, fusion_edge_index, device, x_dict, edge_index_dict):
    """
    Compute P A_f P^T where P is the projection matrix that aggregates cases into counties.
    This aligns with the theoretical framework for comparison with the full heterogeneous Laplacian.
    """
    # First compute the basic fusion Laplacian
    L_f = compute_fusion_laplacian_learned(fusion_features, fusion_edge_index, device)
    if L_f is None:
        return None
    
    # Get dimensions
    num_counties = fusion_features.size(0)
    num_cases = 0
    if 'case' in x_dict:
        num_cases = x_dict['case'].size(0)
    
    total_hetero_nodes = num_counties + num_cases
    
    if num_cases == 0:
        # No cases, so projection is just identity on counties
        # Pad to match heterogeneous dimensions
        padded_L_f = torch.zeros((total_hetero_nodes, total_hetero_nodes), device=device)
        padded_L_f[:num_counties, :num_counties] = L_f
        return padded_L_f
    
    # Construct projection matrix P: (N + M) x N
    # P aggregates cases into their home counties
    P = torch.zeros((total_hetero_nodes, num_counties), device=device)
    
    # Identity mapping for counties (first N rows)
    P[:num_counties, :num_counties] = torch.eye(num_counties, device=device)
    
    # Case-to-county mapping (last M rows)
    if ('case', 'belongs_to', 'county') in edge_index_dict:
        belongs_to_edges = edge_index_dict[('case', 'belongs_to', 'county')]
        case_indices, county_indices = belongs_to_edges
        
        # Offset case indices to start after counties
        case_indices_offset = case_indices + num_counties
        valid_mask = (case_indices_offset < total_hetero_nodes) & (county_indices < num_counties)
        
        if valid_mask.any():
            case_indices_offset = case_indices_offset[valid_mask]
            county_indices = county_indices[valid_mask]
            P[case_indices_offset, county_indices] = 1.0
    
    # Compute P A_f P^T (but we use -L_f since L = D - A)
    A_f = -L_f + torch.diag(torch.diag(L_f))  # A = D - L
    projected_A = torch.mm(torch.mm(P, A_f), P.t())
    
    # Convert back to Laplacian
    projected_deg = torch.diag(torch.sum(projected_A, dim=1))
    projected_L = projected_deg - projected_A
            ...
            if spectral_gamma > 0 and fusion_builder is not None:
                if device_graphs and len(device_graphs[-1]) > i:
                    ...
                    if last_input_graph and hasattr(last_input_graph, 'x_dict') and hasattr(last_input_graph,
                                                                                            'edge_index_dict'):
                        # Compute full heterogeneous Laplacian (includes all node types and edge types)
                        L_het = compute_full_heterogeneous_laplacian(
                        ...
                        try:
                            ...
                            # Compute projected fusion Laplacian P A_f P^T to match dimensions
                            L_fus = compute_projected_fusion_laplacian(
