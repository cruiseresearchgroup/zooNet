...
class SimpleGraphDataset(Dataset):
    ...
    def _load_file_paths(self):
        ...
        graph_files_pt = glob.glob(os.path.join(self.graphs_dir, "hetero_graph_*.pt"))
    ...
    def __getitem__(self, idx):
        ...
        for year, week, file_path in input_seq:
            ...
            # Store both encoded and raw temporal info: (encoded_tensors, raw_values, filename)
            temporal_info.append((year_encoding, week_encoding, year, week, os.path.basename(file_path)))
        ...
        for year, week, file_path in target_seq:
            ...
            year_encoding, week_encoding = self.encode_temporal_info(year, week)
            # Store both encoded and raw temporal info: (encoded_tensors, raw_values, filename)
            target_temporal_info.append((year_encoding, week_encoding, year, week, os.path.basename(file_path)))
            if self.dataset == 'japan':
